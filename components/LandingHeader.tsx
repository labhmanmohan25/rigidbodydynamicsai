...
export default function LandingHeader() {
  ...
  return (
    <header className="fixed top-2 left-0 right-0 z-50 flex justify-center px-2 sm:top-4 sm:px-4">
      <div
        className={`flex w-full max-w-[min(100vw-1rem,var(--max-width-content))] flex-col rounded-xl px-3 py-2 shadow-lg backdrop-blur-sm ring-1 ring-inset md:w-auto md:py-2 ${isHome ? "bg-white/10 ring-white/20" : "bg-white/95 ring-black/10"}`}
        style={{
          boxShadow:
      ...
      >
        ...
        <nav
          ...
          className={`border-t pb-2 pt-3 md:hidden ${menuOpen ? "block" : "hidden"} ${isHome ? "border-white/15" : "border-neutral-200/80"}`}
        >
